...
const DispositionRadar: React.FC<DispositionRadarProps> = ({ profile }) => {
  // Map profile traits to constellation nodes
  // Using explicit values from the text description in previous prompts + cluster stats
  const nodes = [
    { id: 'stem', label: 'STEM', value: profile.clusterStats.stem, angle: 270, color: '#22d3ee' }, // Cyan (Top)
    { id: 'huma', label: 'Humanities', value: profile.clusterStats.humanities, angle: 30, color: '#a855f7' }, // Purple
    { id: 'collab', label: 'Collab', value: 65, angle: 90, color: '#4ade80' }, // Green (Right) - Derived from notes
    { id: 'creative', label: 'Creative', value: 55, angle: 150, color: '#fbbf24' }, // Amber
    { id: 'analyt', label: 'Analytical', value: profile.clusterStats.analytical, angle: 210, color: '#f472b6' }, // Pink
    { id: 'detail', label: 'Detail', value: 73, angle: 330, color: '#94a3b8' }, // Slate - Derived from notes
  ];
  ...
  const getCoordinates = (value: number, angleDegrees: number) => {
    ...
    // Normalize value 0-100 to radius distance (min 20px from center, max radius)
              ...
              return (
                <motion.g 
                  ...
                  animate={{ 
                    opacity: 1, 
                    scale: 1,
                    x: coords.x - center, // We animate from 0,0 relative to parent group, but let's just position absolute in SVG
                    y: coords.y - center
                  }}
                  transition={{ delay: i * 0.1, duration: 0.5, type: "spring" }}
                  style={{ originX: 0, originY: 0 }} // Reset origin for transform
                >
                  {/* We use a nested motion group for the floating effect to separate it from the entrance animation */}
                  <motion.g
                     initial={{ x: center, y: center }} // Start at center for ease
                     animate={{ 
                       x: coords.x, 
                       y: coords.y,
                     }}
